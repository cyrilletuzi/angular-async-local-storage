...
export default function (): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    if (angularMajorVersion === undefined) {
      context.logger.warn(`Not able to detect @angular/core version. Be aware that Angular versions <= 19 are no longer supported.`);
    }

    if (angularMajorVersion !== undefined && (angularMajorVersion <= 19)) {
      throw new SchematicsException("Angular versions <= 19 are no longer supported.");
    }
