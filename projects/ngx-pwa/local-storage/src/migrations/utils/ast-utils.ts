...
export function insertImport(source: ts.SourceFile, fileToEdit: string, symbolName: string,
  ...
  if (relevantImports.length > 0) {
    ...
    if (importTextNodes.length === 0) {
      ...
      return insertAfterLastOccurrence(imports, `, ${symbolName}`, fileToEdit, fallbackPos1);
    }
  ...
  const useStrict = (findNodes(rootNode, ts.SyntaxKind.StringLiteral) as ts.StringLiteral[])
...
export function getDecoratorMetadata(source: ts.SourceFile, identifier: string,
  ...
  const angularImports: {[name: string]: string}
    = (findNodes(source, ts.SyntaxKind.ImportDeclaration) as ts.ImportDeclaration[])
...
export function addSymbolToNgModuleMetadata(
  ...
  if (matchingProperties.length === 0) {
    ...
    let toInsert1: string;
    if (expr.properties.length === 0) {
      position1 = expr.getEnd() - 1;
      toInsert1 = `  ${metadataField}: [${symbolName}]\n`;
    } else {
      node = expr.properties[expr.properties.length - 1];
      position1 = node.getEnd();
      // Get the indentation of the last element, if any.
      const text = node.getFullText(source);
      const matches = text.match(/^\r?\n\s*/);
      if (matches && matches.length > 0) {
        toInsert1 = `,${matches[0]}${metadataField}: [${symbolName}]`;
      } else {
        toInsert1 = `, ${metadataField}: [${symbolName}]`;
      }
    }
    if (importPath !== null) {
      return [
        new InsertChange(ngModulePath, position1, toInsert1),
        insertImport(source, ngModulePath, symbolName.replace(/\..*$/, ''), importPath),
      ];
    } else {
      return [new InsertChange(ngModulePath, position1, toInsert1)];
    }
  }
